...
// 1. Traduzione TESTO -> MORSE (Al clic del pulsante)
document.getElementById('btnConvert').onclick = () => {
    ...
    document.getElementById('outputMorse').value = val.split('').map(c => {
        if (c === ' ') return '/';
        return REVERSE_MAP[c] || '';
    }).join(' ');
};

// 2. Traduzione MANUALE MORSE -> TESTO (Mentre scrivi nel campo Morse)
document.getElementById('inputMorseManual').oninput = (e) => {
    const morseInput = e.target.value.trim();
    if (!morseInput) {
        document.getElementById('outputTranslatedText').value = "";
        return;
    }
    // Traduzione: divide per spazio (lettere) e interpreta '/' come spazio parola
    const translated = morseInput.split(' ').map(code => {
        return MORSE_MAP[code] || (code === '/' ? ' ' : '?');
    }).join('');
    
    document.getElementById('outputTranslatedText').value = translated;
};
